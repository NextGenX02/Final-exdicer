...
import threading
import tldextract
from colorama import init, Fore  # Impor colorama untuk dukungan Windows
requests.packages.urllib3.disable_warnings()

# Inisialisasi colorama untuk kompatibilitas Windows
init(autoreset=True)

# Ganti kode warna dengan colorama
R = Fore.RED    # red
G = Fore.GREEN  # green
C = Fore.CYAN   # cyan
W = Fore.WHITE  # white
...
def url_filter(target):
    ...
    if all([url.find('http://') == -1, url.find('https://') == -1]):
    ...
    if all([url.find('//') == -1, url.find('../') == -1, url.find('./') == -1, url.find('http://') == -1, url.find('https://') == -1]):
        url = target + '/' + url
    else:
        pass
