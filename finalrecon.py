...
def full_recon():
    ...
    dnsrec(hostname, output, data)
    if type_ip == False:
        subdomains(hostname, tout, output, data)
    else:
        pass
...
try:
    ...
    from modules.exports import export
...
if output != 'None':
    # Check if the Directory is Exist in the first place
    if not os.path.exists(os.path.join(os.getcwd(),"dump")):
        mkdir(os.path.join(os.getcwd(),"dump"))
    fname = os.path.join(os.getcwd(), "dump",f"{hostname}.{output}")
